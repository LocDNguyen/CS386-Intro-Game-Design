from ast import Or
import pygame as pg
from pygame.sprite import Sprite, Group
from laser import Lasers
from timer import Timer
from random import choice


class Alien(Sprite):

    alien_images = [pg.transform.rotozoom(pg.image.load(f'images/blue/blue_alien{n}.png'), 0, 1) for n in range(2)]
    alien_images1 = [pg.transform.rotozoom(pg.image.load(f'images/green/green_alien{n}.png'), 0, 1) for n in range(2)]
    alien_images2 = [pg.transform.rotozoom(pg.image.load(f'images/pink/pink_alien{n}.png'), 0, 1) for n in range(2)]

    alien_explosion_images = [pg.transform.rotozoom(pg.image.load(f'images/blue/explode{n}.png'), 0, 1) for n in range(11)]
    alien_explosion_images1 = [pg.transform.rotozoom(pg.image.load(f'images/green/explode{n}.png'), 0, 1) for n in range(11)]
    alien_explosion_images2 = [pg.transform.rotozoom(pg.image.load(f'images/pink/explode{n}.png'), 0, 1) for n in range(11)]

    def __init__(self, game, type):
        super().__init__()
        self.screen = game.screen
        self.settings = game.settings
        self.sb = game.scoreboard
        self.image = pg.image.load('images/blue/blue_alien0.png')
        self.rect = self.image.get_rect()
        self.rect.y = self.rect.height
        self.x = float(self.rect.x)
        self.type = type

        self.dying = self.dead = False

        # self.timer_normal = Timer(image_list=self.alien_images)
        # self.timer_normal = Timer(image_list=self.alien_types[type])

        if self.type == 'blue':
            self.timer_normal = Timer(image_list=self.alien_images, delay=500)
            self.timer_explosion = Timer(image_list=self.alien_explosion_images, delay=90, is_loop=False)
            self.timer = self.timer_normal
        elif self.type == 'green':
            self.timer_normal = Timer(image_list=self.alien_images1, delay=500)
            self.timer_explosion = Timer(image_list=self.alien_explosion_images1, delay=90, is_loop=False)
            self.timer = self.timer_normal
        else:
            self.timer_normal = Timer(image_list=self.alien_images2, delay=500)
            self.timer_explosion = Timer(image_list=self.alien_explosion_images2, delay=90, is_loop=False)
            self.timer = self.timer_normal

    def check_edges(self):
        screen_rect = self.screen.get_rect()
        return self.rect.right >= screen_rect.right or self.rect.left <= 0
    def check_bottom_or_ship(self, ship):
        screen_rect = self.screen.get_rect()
        return self.rect.bottom >= screen_rect.bottom or self.rect.colliderect(ship.rect)
    def hit(self):
        if not self.dying:
            self.dying = True
            self.timer = self.timer_explosion
            if self.type == 'blue':
                self.sb.score += 10
                self.sb.prep_score()
            elif self.type == 'green':
                self.sb.score += 20
                self.sb.prep_score()
            else:
                self.sb.score += 40
                self.sb.prep_score()
    def update(self):
        if self.timer == self.timer_explosion and self.timer.is_expired():
            self.kill()
        settings = self.settings
        self.x += (settings.alien_speed_factor * settings.fleet_direction)
        self.rect.x = self.x
        self.draw()
    def draw(self):
        image = self.timer.image()
        rect = image.get_rect()
        rect.left, rect.top = self.rect.left, self.rect.top
        self.screen.blit(image, rect)
        # self.screen.blit(self.image, self.rect)


class Aliens:
    def __init__(self, game, alien_lasers):
        self.model_alien = Alien(game=game, type='blue')
        self.game = game
        self.sb = game.scoreboard
        self.aliens = Group()

        self.ship_lasers = game.ship_lasers.lasers
        self.alien_lasers = alien_lasers
        self.barriers = game.barriers.barriers

        self.screen = game.screen
        self.settings = game.settings
        self.sound = game.sound
        self.ship = game.ship
        self.create_fleet()
    def get_number_aliens_x(self, alien_width):
        available_space_x = self.settings.screen_width - 6 * alien_width
        number_aliens_x = int(available_space_x / (1 * alien_width))
        return number_aliens_x
    def get_number_rows(self, ship_height, alien_height):
        # available_space_y = (self.settings.screen_height - (3 * alien_height) - ship_height)
        # number_rows = int(available_space_y / (2 * alien_height))
        number_rows = 5
        return number_rows
    def reset(self):
        self.aliens.empty()
        self.create_fleet()
        self.alien_lasers.reset()
    def create_alien(self, alien_number, row_number, type):
        alien = Alien(game=self.game, type=type)
        alien_width = alien.rect.width

        alien.x = alien_width + 1.5 * alien_width * alien_number
        alien.rect.x = alien.x
        alien.rect.y = alien.rect.height + 1 * alien.rect.height * row_number
        self.aliens.add(alien)
    def create_fleet(self):
        number_aliens_x = self.get_number_aliens_x(self.model_alien.rect.width)
        number_rows = self.get_number_rows(self.ship.rect.height, self.model_alien.rect.height)
        for row_number in range(number_rows):
            for alien_number in range(number_aliens_x):
                if row_number <= 1:
                    self.create_alien(alien_number, row_number, 'blue')
                elif row_number > 3:
                    self.create_alien(alien_number, row_number, 'pink')
                else:
                    self.create_alien(alien_number, row_number, 'green')
    def check_fleet_edges(self):
        for alien in self.aliens.sprites():
            if alien.check_edges():
                self.change_fleet_direction()
                break
    def check_fleet_bottom(self):
        for alien in self.aliens.sprites():
            if alien.check_bottom_or_ship(self.ship):
                self.ship.die()
    def check_fleet_empty(self):
        if len(self.aliens.sprites()) == 0:
            print('Aliens all gone!')
            self.sb.level += 1
            self.game.reset()
    def change_fleet_direction(self):
        for alien in self.aliens.sprites():
            alien.rect.y += self.settings.fleet_drop_speed
        self.settings.fleet_direction *= -1
    def check_collisions(self):
        collisions = pg.sprite.groupcollide(self.aliens, self.ship_lasers, False, True)
        if collisions:
            for alien in collisions:
                alien.hit()
        for alien in self.aliens:
            pg.sprite.spritecollide(alien, self.barriers, True)
    def fire(self):
        if self.aliens.sprites():
            firing_alien = choice(self.aliens.sprites())
            self.alien_lasers.shoot(alien=firing_alien)
    def speedup(self):
        if len(self.aliens) == 40:
            self.sound.speedup()
        if len(self.aliens) == 20:
            self.sound.speedup2()
    def update(self):
        self.check_fleet_edges()
        self.check_fleet_bottom()
        self.check_collisions()
        self.check_fleet_empty()
        self.fire()
        self.speedup()
        for alien in self.aliens.sprites():
            if alien.dead:      # set True once the explosion animation has completed
                alien.remove()
            alien.update()
        self.alien_lasers.update()
    def draw(self):
        for alien in self.aliens.sprites():
            alien.draw()
